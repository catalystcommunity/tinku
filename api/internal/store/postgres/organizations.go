...
// organizationColumnsSQL joins both roster counts in as scalar subqueries rather
// than as GROUP BY aggregates: two counts over two different predicates
// would otherwise need two LEFT JOINs and a DISTINCT, and this reads as
// what it is.
...
func scanOrganization(row rowScanner) (*store.Organization, error) {
	...
	err := row.Scan(&g.ID, &g.Slug, &g.OriginDomain, &g.Name, &g.Blurb, &g.Description,
		&publish, &g.CreatedAt, &g.UpdatedAt, &g.MemberCount, &g.OwnerCount)
	if err != nil {
...
// organization with no owner can never be edited and can never be given one, so
// the two writes are not separable.
...
VALUES ($1, $2, $3, $4, $5, $6, $7, $8)`,
	...
	organization, err := scanOrganization(tx.QueryRowContext(ctx, organizationColumnsSQL+` WHERE g.id = $1`, id))
	if err != nil {
		return nil, err
	}
...
// UpdateOrganization replaces every own field. The slug is deliberately not among
// them: it is half of the organization's federated address, and an address that
// moves when somebody renames an organization is not an address.
func (s *Store) UpdateOrganization(ctx context.Context, id string, in store.OrganizationInput) (*store.Organization, error) {
	res, err := s.db.ExecContext(ctx, `
UPDATE organizations SET name = $1, blurb = $2, description = $3, search_text = $4,
...
func (s *Store) OrganizationByID(ctx context.Context, id string) (*store.Organization, error) {
	return scanOrganization(s.db.QueryRowContext(ctx, organizationColumnsSQL+` WHERE g.id = $1`, id))
}
...
func (s *Store) OrganizationBySlug(ctx context.Context, originDomain, slug string) (*store.Organization, error) {
	return scanOrganization(s.db.QueryRowContext(ctx,
		organizationColumnsSQL+` WHERE g.origin_domain = $1 AND g.slug = $2`, originDomain, slug))
}
...
func (s *Store) RemoveOrganizationMember(ctx context.Context, organizationID, userID string) error {
	if _, err := s.db.ExecContext(ctx,
		`DELETE FROM organization_members WHERE organization_id = $1 AND user_id = $2`, organizationID, userID); err != nil {
...
func (s *Store) ListOrganizationMembers(ctx context.Context, organizationID string, page store.Page) ([]store.OrganizationMember, int64, error) {
	...
	if err := s.db.QueryRowContext(ctx,
		`SELECT count(*) FROM organization_members WHERE organization_id = $1`, organizationID).Scan(&total); err != nil {
...
func (s *Store) OrganizationRoleFor(ctx context.Context, organizationID, userID string) (store.OrganizationRole, bool, error) {
	...
	err := s.db.QueryRowContext(ctx,
		`SELECT role FROM organization_members WHERE organization_id = $1 AND user_id = $2`, organizationID, userID).Scan(&role)
