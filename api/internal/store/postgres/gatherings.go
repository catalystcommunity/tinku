...
//
// next_event_at reads the clock in SQL rather than taking a bind. A bind
// here has to live in the same argument list as the WHERE clause, and the
// count query that shares that clause does NOT contain this subquery — so
// the parameter is unused there, which Postgres rejects and SQLite
// silently misbinds. No caller pins this clock, so nothing is lost.
...
func scanGathering(row rowScanner) (*store.Gathering, error) {
	...
	err := row.Scan(&g.ID, &g.Slug, &g.OriginDomain, &g.Name, &g.Blurb, &g.Description,
		&publish, &g.CreatedAt, &g.UpdatedAt, &g.MemberCount, &g.EventCount, &g.NextEventAt)
	if err != nil {
...
VALUES ($1, $2, $3, $4, $5, $6, $7, $8)`,
	...
	if _, err := tx.ExecContext(ctx,
		`INSERT INTO gathering_owners (id, gathering_id, user_id, organization_id) VALUES ($1, $2, $3, $4)`,
...
UPDATE gatherings SET name = $1, blurb = $2, description = $3, search_text = $4,
                      publish_events = $5, updated_at = now()
WHERE id = $6`, in.Name, in.Blurb, in.Description, in.SearchText,
...
func (s *Store) GatheringByID(ctx context.Context, id string) (*store.Gathering, error) {
	g, err := scanGathering(s.db.QueryRowContext(ctx, gatheringColumnsSQL+` WHERE gt.id = $1`, id))
	if err != nil {
		return nil, err
	}
...
func (s *Store) GatheringBySlug(ctx context.Context, originDomain, slug string) (*store.Gathering, error) {
	return scanGathering(s.db.QueryRowContext(ctx,
		gatheringColumnsSQL+` WHERE gt.origin_domain = $1 AND gt.slug = $2`,
		originDomain, slug))
}
...
func (s *Store) ListGatherings(ctx context.Context, f store.GatheringFilter) ([]store.Gathering, int64, error) {
	...
	if f.MemberID != "" {
		...
		// an organization whose roster names the caller as an owner. Three ways in,
		// one predicate, because a gathering the caller owns but never
		// joined is still theirs.
		id := a.next(f.MemberID)
		where = append(where, `(
    EXISTS (SELECT 1 FROM gathering_members m WHERE m.gathering_id = gt.id AND m.user_id = `+id+`)
 OR EXISTS (SELECT 1 FROM gathering_owners o WHERE o.gathering_id = gt.id AND o.user_id = `+id+`)
 OR EXISTS (SELECT 1 FROM gathering_owners o
              JOIN organization_members gm ON gm.organization_id = o.organization_id
            WHERE o.gathering_id = gt.id AND gm.user_id = `+id+` AND gm.role = 'owner'))`)
	}
	...
	if place := f.Place; place.Locality != "" || place.Region != "" || place.Country != "" || place.Box != nil {
		...
		// event under it does — which is what somebody searching "climbing
		// in Denver" is actually asking.
...
func (s *Store) DeleteGathering(ctx context.Context, id string) error {
	if _, err := s.db.ExecContext(ctx, `DELETE FROM gatherings WHERE id = $1`, id); err != nil {
...
// whichever of the two applies, the conflict target is the one that names
// the column actually set.
...
func (s *Store) RemoveGatheringOwner(ctx context.Context, gatheringID string, owner store.OwnerRef) error {
	...
	if _, err := s.db.ExecContext(ctx,
		`DELETE FROM gathering_owners WHERE gathering_id = $1 AND `+column+` = $2`,
...
func (s *Store) CountGatheringOwners(ctx context.Context, gatheringID string) (int64, error) {
	...
	if err := s.db.QueryRowContext(ctx,
		`SELECT count(*) FROM gathering_owners WHERE gathering_id = $1`, gatheringID).Scan(&n); err != nil {
...
func (s *Store) JoinGathering(ctx context.Context, gatheringID, userID string) error {
	_, err := s.db.ExecContext(ctx, `
INSERT INTO gathering_members (gathering_id, user_id) VALUES ($1, $2)
...
func (s *Store) LeaveGathering(ctx context.Context, gatheringID, userID string) error {
	if _, err := s.db.ExecContext(ctx,
		`DELETE FROM gathering_members WHERE gathering_id = $1 AND user_id = $2`,
...
func (s *Store) ListGatheringMembers(ctx context.Context, gatheringID string, page store.Page) ([]store.GatheringMember, int64, error) {
	var total int64
	if err := s.db.QueryRowContext(ctx,
		`SELECT count(*) FROM gathering_members WHERE gathering_id = $1`, gatheringID).Scan(&total); err != nil {
...
WHERE m.gathering_id = $1
ORDER BY m.joined_at, u.handle
LIMIT $2 OFFSET $3`, gatheringID, limit, offset)
	if err != nil {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&m.GatheringID, &m.UserID, &m.Handle, &m.LinkkeysDomain,
			&m.DisplayName, &m.JoinedAt); err != nil {
...
// trip. Ownership is three separate reachability questions — direct
...
SELECT
  EXISTS (SELECT 1 FROM gathering_members m WHERE m.gathering_id = $1 AND m.user_id = $2),
  EXISTS (SELECT 1 FROM gathering_owners o WHERE o.gathering_id = $1 AND o.user_id = $2)
    OR EXISTS (SELECT 1 FROM gathering_owners o
                 JOIN organization_members gm ON gm.organization_id = o.organization_id
               WHERE o.gathering_id = $1 AND gm.user_id = $2 AND gm.role = 'owner')`,
		gatheringID, userID).Scan(&access.IsMember, &access.IsOwner)
	if err != nil {
